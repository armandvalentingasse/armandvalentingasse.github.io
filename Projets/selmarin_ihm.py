...
class DBManagerPro:
    ...
    def show_import_csv(self):
        ...
        # Bouton de suppression (visible seulement pour l'admin)
    ...
    def show_sql_console(self):
        ...
        ttk.Button(btn_frame, text="Exécuter (Ctrl+Entrée)", 
                  style="Accent.TButton",
                  command=self.execute_sql).pack(side=tk.LEFT)
        
        examples_frame = ttk.Frame(main_frame, style="Card.TFrame", padding=8)
        examples_frame.pack(fill=tk.X, pady=(0, 10))
        
        ttk.Label(examples_frame, text="Exemples rapides :").pack(anchor="w")
        
        examples = [
            ("Tables", "SHOW TABLES"),
            ("Structure", "DESCRIBE ma_table"),
            ("Sélection", "SELECT * FROM ma_table LIMIT 10"),
            ("Insertion", "INSERT INTO ma_table (champ) VALUES ('valeur')"),
            ("Mise à jour", "UPDATE ma_table SET champ='valeur' WHERE id=1")
        ]
        
        example_btns = ttk.Frame(examples_frame)
        example_btns.pack(fill=tk.X)
        
        for text, cmd in examples:
            btn = ttk.Button(example_btns, text=text,
                            command=lambda c=cmd: self.insert_sql_command(c),
                            width=10)
    ...
    def insert_sql_command(self, command):
        ...
        self.sql_input.insert(tk.END, command)
